...
#
# Set up vip_avalon_mm_part_path and default_library
#------------------------------------------------------
...
if {$argc >= 2} {
  ...
  quietly set destination_path vip_avalon_mm_part_path
...
#--------------------------------------------------
quietly set fp [open "$vip_avalon_mm_part_path/script/compile_order.txt" r]
...
foreach item $file_data {
  if {$idx > 2} {
    echo "eval vcom  $compdirectives  $vip_avalon_mm_part_path/sim/$item"
    eval vcom  $compdirectives  $vip_avalon_mm_part_path/sim/$item
  }
  incr idx 1
}
