...
sub RunDeployToCustomer {
  ...
  if ($ProductName =~ /uvvm_util/i ) {
    ...
    chdir "uvvm_util/sim" or die "Could not chdir to uvvm_util/sim: $!";
  
    # Test compilation of Util
    ...
    # Simulate Util with the irqc DUT
    chdir "../../bitvis_irqc/sim"  or die "Could not chdir to ../../bitvis_irq/sim: $!";
    print("Now we run IRQC simulations to verify that Util simulates...\n"); 
  

    PerlUtil::PrintConfirmAndRun("vsim -c -do \"do ../script/compile_and_sim_all.do; exit -f -code 0\"", $DryRun);
...
sub FinaliseRelease {
  ...
  # **********************************************************************************************
  # WORK-AROUND: Rename the final zip file to UVVM_Utility_Library_v<x>_<x>_<x>.zip
  ...
  my $ZipProductName = "UVVM_Utility_Library";
  $ZipRevNumber =~ tr/./_/; # replace . with _
  my $old_zip = "./uvvm_utilv${ZipRevNumber}.zip";
  my $new_zip = "./${ZipProductName}_v${ZipRevNumber}.zip";

  if (-f $old_zip)
  {
    print "Renaming final release zip file from uvvm_util_release_v${ZipRevNumber}\n to ${ZipProductName}_v${ZipRevNumber}\n";
