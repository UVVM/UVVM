...
def add_architecture_declaration(file_handle, vvc_name, vvc_channel):
    ...
    if vvc_channel.name == "NA":
      ...
      file_handle.write("  alias dtt_transaction_info    : t_transaction_group is global_" + vvc_name.lower() + "_vvc_transaction(GC_INSTANCE_IDX);\n")
      ...
      file_handle.write("    -- DTT\n")
      file_handle.write("  alias dtt_transaction_info    : t_transaction_group is global_" + vvc_name.lower() + "_vvc_transaction(GC_CHANNEL, GC_INSTANCE_IDX);\n")
...
def add_vvc_executor(file_handle, vvc_channel):
    ...
    file_handle.write("      restore_global_dtt(dtt_transaction_info, v_cmd);\n")
    print_linefeed(file_handle)
    file_handle.write("    end loop;\n")
    file_handle.write("  end process;\n")
    file_handle.write(division_line+"\n")
...
def add_methods_pkg_body(file_handle, vvc_name):
    ...
    file_handle.write("    signal dtt_group    : inout t_transaction_group ;\n")
    file_handle.write("    constant vvc_cmd    : in t_vvc_cmd_record;\n")
    file_handle.write("    constant vvc_config : in t_vvc_config) is\n")
    file_handle.write("  begin\n")
    file_handle.write("  --   case vvc_cmd.operation is\n")
    file_handle.write("  --     -- Example, fill in t_operation enumerations\n")
    file_handle.write("  --     when TRANSMIT | RECEIVE | EXPECT =>\n")
    ...
    print_linefeed(file_handle)
    file_handle.write("    wait for 0 ns;\n")
    file_handle.write("  end procedure restore_global_dtt;\n")
    print_linefeed(file_handle)
...
def add_transaction_pkg(file_handle, vvc_name, vvc_channels):
    ...
    file_handle.write("    );\n")
    print_linefeed(file_handle)
    file_handle.write("  -- Transaction groups array\n")
    if vvc_channels.__len__() == 1:
      file_handle.write("  type t_" + vvc_name.lower() + "_transaction_group_array is array (natural range <>) of t_transaction_group;\n")
      print_linefeed(file_handle)
      file_handle.write("  -- Global DTT signals\n")
      file_handle.write("  signal global_" + vvc_name.lower() + "_vvc_transaction : t_" + vvc_name.lower() + "_transaction_group_array(0 to C_MAX_VVC_INSTANCE_NUM) :=\n")
      file_handle.write("    (others => C_TRANSACTION_GROUP_DEFAULT);\n")
    else:
      file_handle.write("  type t_" + vvc_name.lower() + "_transaction_group_array is array (t_channel range <>, natural range <>) of t_transaction_group;\n")
      print_linefeed(file_handle)
      file_handle.write("  -- Global DTT signals\n")
      file_handle.write("  signal global_" + vvc_name.lower() + "_vvc_transaction : t_" + vvc_name.lower() + "_transaction_group_array(t_channel'left to t_channel'right, 0 to C_MAX_VVC_INSTANCE_NUM) :=\n")
