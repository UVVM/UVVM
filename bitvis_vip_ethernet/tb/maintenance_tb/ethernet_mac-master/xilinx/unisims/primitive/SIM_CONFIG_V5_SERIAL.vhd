...
-- /___/   /\     Filename : SIM_CONFIG_V5_SERIAL.vhd
-- \   \  /  \    Timestamp : 
--  \___\/\___\
--
-- Revision:
...
architecture SIM_CONFIG_V5_SERIAL_V of SIM_CONFIG_V5_SERIAL is
   ...
   rdwr_b_in <= '0';
        ...
        begin
          if (DEVICE_ID = X"00000000") then
             assert FALSE report "Attribute Syntax Error : The attribute DEVICE_ID on  SIM_CONFIG_V5_SERIAL is not set." severity error;
          end if;
          wait;
        end process;
...
--   wait on rst_en, init_rst, prog_b_in, iprog_b;
      ...
      if (mode_sample_flag = '0') then
        if(prog_b_t = '1') then
           ...
           if (m_in /= "111") then
              ...
              Write ( Message, string'(" . Only Slave SERIAL  mode M=111 supported on SIM_CONFIG_V5_SERIAL."));
              assert false report Message.all severity error;
              DEALLOCATE (Message);
           end if;
        elsif (NOW > 0 ps) then
            assert false report "Error: PROGB is not high when INITB goes high on SIM_CONFIG_V5_SERIAL." severity error;
       end if;
    end if;
   end if;
  end process;
  ...
  process (m_in) begin
    if (mode_sample_flag = '1' and persist_en = '1') then
       assert false report "Error : Mode pine M[2:0] changed after rising edge of INITB on SIM_CONFIG_V5_SERIAL." severity error;
    end if;
  end process;
    ...
    begin
      if (rst_intl = '0'  or  abort_flag_rst = '1'  or  csi_b_in  =  '1') then
        ...
        if (abort_dis  =  '0'  and  csi_b_in  =  '0') then
             if (NOW  /=  0 ps) then
               abort_flag_wr <= '1';
               assert false report " Warning : RDWRB changes when CS_B low, which causes Configuration abort on SIM_CONFIG_V5_SERIAL."
               severity warning;
             end if;
        else
           abort_flag_wr <= '0';
