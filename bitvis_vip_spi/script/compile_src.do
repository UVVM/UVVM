...
#
# Set up vip_spi_part_path and default_library
#------------------------------------------------------
...
if {$argc >= 2} {
  ...
  quietly set destination_path vip_spi_part_path
...
#--------------------------------------------------
quietly set fp [open "$vip_spi_part_path/script/compile_order.txt" r]
...
if { [string equal -nocase $simulator "modelsim"] } {
  ...
  set compdirectives "-relax -2008 -nowarn COMP96_0564 -nowarn COMP96_0048 -dbg -work $lib_name"
}
...
foreach item $file_data {
  if {$idx > 2} {
    echo "eval vcom  $compdirectives  $vip_spi_part_path/sim/$item"
    eval vcom  $compdirectives  $vip_spi_part_path/sim/$item
  }
  incr idx 1
}
