...
def add_vvc_interpreter(file_handle, vvc_channel):
    ...
    file_handle.write("      -- Select between a provided msg_id_panel from the test sequencer via a command or the default\n")
    file_handle.write("      -- VVC msg_id_panel in the config. This is to correctly handle the logging when using HVVCs.\n")
...
def add_vvc_executor(file_handle, vvc_channel):
    ...
    if number_of_executors > 1:
        ...
        file_handle.write("        --       shared_"+vvc_name.lower()+"_sb.check_received(GC_INSTANCE_IDX, v_result(GC_DATA_WIDTH-1 downto 0));\n")
        file_handle.write("        --     else\n")
...
def add_vvc_cmd_pkg_header(file_handle):
    ...
    file_handle.write("    use_provided_msg_id_panel : t_use_provided_msg_id_panel;\n")
    ...
    file_handle.write("    use_provided_msg_id_panel => DO_NOT_USE_PROVIDED_MSG_ID_PANEL,\n")
...
def add_methods_pkg_header(file_handle, vvc_name, vvc_channels):
    ...
    file_handle.write("  --   constant scope              : in string\n")
    file_handle.write("  -- );\n")
    print_linefeed(file_handle)
    ...
    file_handle.write("  --==============================================================================\n")
    file_handle.write("  -- Direct Transaction Transfer methods\n")
    file_handle.write("  --==============================================================================\n")
    file_handle.write("  procedure set_global_dtt(\n")
    file_handle.write("    signal dtt_trigger    : inout std_logic;\n")
    file_handle.write("    variable dtt_group    : inout t_transaction_group;\n")
    ...
    print_linefeed(file_handle)
    file_handle.write("  procedure reset_dtt_info(\n")
    file_handle.write("    variable dtt_group    : inout t_transaction_group;\n")
    file_handle.write("    constant vvc_cmd      : in t_vvc_cmd_record);\n")
    print_linefeed(file_handle)
...
def add_methods_pkg_body(file_handle, vvc_name):
    ...
    file_handle.write("  --   send_command_to_vvc(VVCT, scope => scope);\n")
    ...
    file_handle.write("  --    when WRITE | READ | CHECK =>\n")
    file_handle.write("  --      dtt_group.bt.operation                                  := vvc_cmd.operation;\n")
    file_handle.write("  --      dtt_group.bt.address(vvc_cmd.addr'length-1 downto 0)    := vvc_cmd.addr;\n")
    file_handle.write("  --      dtt_group.bt.data(vvc_cmd.data'length-1 downto 0)       := vvc_cmd.data;\n")
    file_handle.write("  --      dtt_group.bt.vvc_meta.msg(1 to vvc_cmd.msg'length)      := vvc_cmd.msg;\n")
    file_handle.write("  --      dtt_group.bt.vvc_meta.cmd_idx                           := vvc_cmd.cmd_idx;\n")
    file_handle.write("  --      dtt_group.bt.transaction_status                         := IN_PROGRESS;\n")
    file_handle.write("  --      gen_pulse(dtt_trigger, 0 ns, \"pulsing global DTT trigger\", scope, ID_NEVER);\n")
    print_linefeed(file_handle)
    file_handle.write("  --    when POLL_UNTIL =>\n")
    file_handle.write("  --      dtt_group.ct.operation                                  := vvc_cmd.operation;\n")
    file_handle.write("  --      dtt_group.ct.address(vvc_cmd.addr'length-1 downto 0)    := vvc_cmd.addr;\n")
    file_handle.write("  --      dtt_group.ct.data(vvc_cmd.data'length-1 downto 0)       := vvc_cmd.data;\n")
    file_handle.write("  --      dtt_group.ct.vvc_meta.msg(1 to vvc_cmd.msg'length)      := vvc_cmd.msg;\n")
    file_handle.write("  --      dtt_group.ct.vvc_meta.cmd_idx                           := vvc_cmd.cmd_idx;\n")
    file_handle.write("  --      dtt_group.ct.transaction_status                         := IN_PROGRESS;\n")
    file_handle.write("  --      gen_pulse(dtt_trigger, 0 ns, \"pulsing global DTT trigger\", scope, ID_NEVER);\n")
    print_linefeed(file_handle)
    file_handle.write("  --    when others =>\n")
    ...
    file_handle.write("  end procedure set_global_dtt;\n")
    print_linefeed(file_handle)
    ...
    file_handle.write("  --  case vvc_cmd.operation is\n")
    file_handle.write("  --    when WRITE | READ | CHECK =>\n")
    file_handle.write("  --      dtt_group.bt := C_TRANSACTION_SET_DEFAULT;\n")
    print_linefeed(file_handle)
    file_handle.write("  --    when POLL_UNTIL =>\n")
    file_handle.write("  --      dtt_group.ct := C_TRANSACTION_SET_DEFAULT;\n")
    print_linefeed(file_handle)
    file_handle.write("  --    when others =>\n")
    ...
    print_linefeed(file_handle)
    file_handle.write("    wait for 0 ns;\n")
    file_handle.write("  end procedure reset_dtt_info;\n")
...
def add_transaction_pkg(file_handle, vvc_name, vvc_channels):
    ...
    file_handle.write("  --==========================================================================================\n")
    file_handle.write("  --\n")
    file_handle.write("  -- DTT - Direct Transaction Transfer types, constants and global signal\n")
    file_handle.write("  --\n")
    file_handle.write("  --==========================================================================================\n")
    print_linefeed(file_handle)
    ...
    file_handle.write("    );\n")
    print_linefeed(file_handle)
    file_handle.write("  -- Transaction\n")
    file_handle.write("  type t_transaction is record\n")
    file_handle.write("    operation           : t_operation;\n")
    file_handle.write("    --<USER_INPUT> Insert transaction information here.\n")
    file_handle.write("    --address             : unsigned(C_VVC_CMD_ADDR_MAX_LENGTH-1 downto 0);  -- Max width may be increased if required\n")
    file_handle.write("    --data                : std_logic_vector(C_VVC_CMD_DATA_MAX_LENGTH-1 downto 0);\n")
    file_handle.write("    vvc_meta            : t_vvc_meta;\n")
    file_handle.write("    transaction_status  : t_transaction_status;\n")
    file_handle.write("  end record;\n")
    print_linefeed(file_handle)
    file_handle.write("  constant C_TRANSACTION_SET_DEFAULT : t_transaction := (\n")
    file_handle.write("    operation           => NO_OPERATION,\n")
    file_handle.write("    --<USER_INPUT> Insert transaction information here.\n")
    ...
    file_handle.write("    bt : t_transaction;\n")
    file_handle.write("    --ct : t_transaction;\n")
    file_handle.write("  end record;\n")
    print_linefeed(file_handle)
    file_handle.write("  constant C_TRANSACTION_GROUP_DEFAULT : t_transaction_group := (\n")
    file_handle.write("    bt => C_TRANSACTION_SET_DEFAULT\n")
    file_handle.write("    --ct => C_TRANSACTION_SET_DEFAULT\n")
    file_handle.write("    );\n")
    print_linefeed(file_handle)
    if vvc_channels.__len__() == 1:
...
def generate_transaction_pkg_file(vvc_name, vvc_channels):
