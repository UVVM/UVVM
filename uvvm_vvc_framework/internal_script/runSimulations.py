...
if __name__ == '__main__':
    ...
    if (re.match(".*" + re.escape(os.path.join(myProjectName, "sim")),os.getcwd())):
        ...
        pyUtil.PrintAndRun('vsim -c -do "do ../script/compile_util.do; exit -f -code 0" | tee sim.log', 0, runID, releaseVars);
        ...
        os.chdir(os.path.join('..', '..', 'bitvis_uart', 'sim'))  
        if (re.match(".*" + re.escape(os.path.join("bitvis_uart","sim")),os.getcwd())):
            print("Now we run simulations to verify that %s simulates...\n" % myProjectName)
            pyUtil.PrintAndRun('vsim -c -do \"do ../script/compile_all_and_sim_uart_vvc_tb.do; exit -f -code 0\"', 0, 0, releaseVars)# runID changed for debug purposes###runID, releaseVars) 
            ...
            # Verify that the wave.do file is OK
            # if {[catch {eval "source ../script/wave_uart_vvc_tb.do"} message]} { 
            #   puts $message
            #   quit -force -code 1
            # }
            print("Verifying wave_uart_vvc_tb.do\n")
            pyUtil.PrintAndRun('vsim -c -do \"do ../internal_script/internal_verify_wave_uart_vvc_tb.do; ' 
                               'exit -f -code 0\"', 0, 0, releaseVars)

            # Go up three levels
            os.chdir(os.path.join('..', '..', '..'))
        else:
            pyUtil.exceptionHandler("Could not chdir to ../../bitvis_uart/sim: $!", 0, releaseVars)# runID changed for debug purposes###runID, releaseVars)  
    else:
        pyUtil.exceptionHandler("Could not chdir to uvvm_vvc_framework/sim:", 0, releaseVars)# runID changed for debug purposes###runID, releaseVars) 
