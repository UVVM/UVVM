...
def create_config(addr_widths, data_widths):
    config = []
    for addr_width, data_width in product(addr_widths, data_widths):
        config.append([str(addr_width),
                       str(data_width)])
    return config


print('Verify External VIP APB')
...
for config in create_config(addr_widths=[7], data_widths=[8]):
    hr.add_generics(entity="apb_bfm_tb", generics=["GC_ADDR_WIDTH", config[0], "GC_DATA_WIDTH", config[1]])
for config in create_config(addr_widths=[8], data_widths=[16]):
    hr.add_generics(entity="apb_bfm_tb", generics=["GC_ADDR_WIDTH", config[0], "GC_DATA_WIDTH", config[1]])
for config in create_config(addr_widths=[32], data_widths=[32]):
    hr.add_generics(entity="apb_bfm_tb", generics=["GC_ADDR_WIDTH", config[0], "GC_DATA_WIDTH", config[1]])

# Setup VVC TB test generics
for config in create_config(addr_widths=[7], data_widths=[8]):
    hr.add_generics(entity="apb_vvc_tb", generics=["GC_ADDR_WIDTH", config[0], "GC_DATA_WIDTH", config[1]])
for config in create_config(addr_widths=[8], data_widths=[16]):
    hr.add_generics(entity="apb_vvc_tb", generics=["GC_ADDR_WIDTH", config[0], "GC_DATA_WIDTH", config[1]])
for config in create_config(addr_widths=[32], data_widths=[32]):
    hr.add_generics(entity="apb_vvc_tb", generics=["GC_ADDR_WIDTH", config[0], "GC_DATA_WIDTH", config[1]])
