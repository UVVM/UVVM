...
* `BFM`_
  ...
  * :ref:`rgmii_write_bfm`
  * :ref:`rgmii_read_bfm`
  * :ref:`rgmii_expect_bfm`
  * :ref:`init_rgmii_if_signals_bfm`

* `VVC`_

  * :ref:`rgmii_write_vvc`
  * :ref:`rgmii_read_vvc`
  * :ref:`rgmii_expect_vvc`
...
+------------------------------+------------------------------+-----------------+-------------------------------------------------+
| rx_clock_skew                | time                         | C_UNDEFINED_TIME| Skew of the sampling of the data in connection  |
|                              |                              |                 | to the RX clock edges. Suggested value is       |
|                              |                              |                 | clock_period/4.                                 |
+------------------------------+------------------------------+-----------------+-------------------------------------------------+
...
|                              |                              |                 | the BFM                                         |
+------------------------------+------------------------------+-----------------+-------------------------------------------------+
| data_valid_on_both_clock_edg\| boolean                      | true            | Switch for changing between double data rate and|
| es                           |                              |                 | single data rate on RGMII interface             |
+------------------------------+------------------------------+-----------------+-------------------------------------------------+
...
.. note::
    ...
    The BFM configuration has to be defined and used when calling the RGMII BFM procedures. See :ref:`rgmii_local_bfm_config` for 
    an example of how to define a local BFM config.
...
While config parameter data_valid_on_both_clock_edges = false:
  * Writes 4 bits of data on each rising clock edge. The bits [3:0] are written on the first rising edge and the bits [7:4] on the 
    following rising edge.

The length and data are defined by the "data_array" argument, which is a t_byte_array. data_array(0) is written first, while 
...
+==========+====================+========+==============================+=========================================================+
| constant | data_array         | in     | :ref:`t_byte_array`          | An array of bytes containing the data to be written     |
+----------+--------------------+--------+------------------------------+---------------------------------------------------------+
...
+----------+--------------------+--------+------------------------------+---------------------------------------------------------+
| signal   | rgmii_tx_if        | inout  | :ref:`t_rgmii_tx_if          | RGMII TX signal interface record                        |
|          |                    |        | <t_rgmii_if>`                |                                                         |
+----------+--------------------+--------+------------------------------+---------------------------------------------------------+
| constant | scope              | in     | string                       | Describes the scope from which the log/alert originates.|
|          |                    |        |                              | Default value is C_BFM_SCOPE ("RGMII BFM").             |
+----------+--------------------+--------+------------------------------+---------------------------------------------------------+
| constant | msg_id_panel       | in     | t_msg_id_panel               | Controls verbosity within a specified scope. Default    |
|          |                    |        |                              | value is shared_msg_id_panel. For more information see  |
|          |                    |        |                              | :ref:`vvc_framework_verbosity_ctrl`.                    |
+----------+--------------------+--------+------------------------------+---------------------------------------------------------+
| constant | config             | in     | :ref:`t_rgmii_bfm_config     | Configuration of BFM behavior and restrictions. Default |
|          |                    |        | <t_rgmii_bfm_config>`        | value is C_RGMII_BFM_CONFIG_DEFAULT.                    |
+----------+--------------------+--------+------------------------------+---------------------------------------------------------+
...
----------------------------------------------------------------------------------------------------------------------------------
While config parameter data_valid_on_both_clock_edges = true (default):
  * Reads 4 bits of data on each clock edge. The bits [3:0] are read on the rising edge and the bits [7:4] on the falling edge. To 
    avoid having to delay the receiver's clock, the config rx_clock_skew is used to set the sampling time of the data.

While config parameter data_valid_on_both_clock_edges = false:
  * Reads 4 bits of data on each rising clock edge. The bits [3:0] are read on the first rising edge and the bits [7:4] on the 
    following rising edge.

The received data is stored in the data_array output, which is a t_byte_array. The number of valid bytes in the data_array is 
stored in data_len. data_array(0) is read first, while data_array(data_array'high) is read last.
...
| Object   | Name               | Dir.   | Type                         | Description                                             |
+==========+====================+========+==============================+=========================================================+
| variable | data_array         | out    | :ref:`t_byte_array`          | An array of bytes containing the data to be read        |
+----------+--------------------+--------+------------------------------+---------------------------------------------------------+
...
| constant | msg                | in     | string                       | A custom message to be appended in the log/alert        |
+----------+--------------------+--------+------------------------------+---------------------------------------------------------+
| signal   | rgmii_rx_if        | inout  | :ref:`t_rgmii_rx_if          | RGMII RX signal interface record                        |
|          |                    |        | <t_rgmii_if>`                |                                                         |
+----------+--------------------+--------+------------------------------+---------------------------------------------------------+
| constant | scope              | in     | string                       | Describes the scope from which the log/alert originates.|
|          |                    |        |                              | Default value is C_BFM_SCOPE ("RGMII BFM").             |
...
rgmii_expect()
----------------------------------------------------------------------------------------------------------------------------------
Calls the :ref:`rgmii_read_bfm` procedure, then compares the read data with data_exp. If the read data is not equal to the 
...
| Object   | Name               | Dir.   | Type                         | Description                                             |
+==========+====================+========+==============================+=========================================================+
| constant | data_exp           | in     | :ref:`t_byte_array`          | An array of bytes containing the data to be expected    |
+----------+--------------------+--------+------------------------------+---------------------------------------------------------+
| constant | msg                | in     | string                       | A custom message to be appended in the log/alert        |
+----------+--------------------+--------+------------------------------+---------------------------------------------------------+
| signal   | rgmii_rx_if        | inout  | :ref:`t_rgmii_rx_if          | RGMII RX signal interface record                        |
...
.. _rgmii_local_bfm_config:

Local BFM configuration
==================================================================================================================================
The RGMII BFM requires that a local configuration is declared in the testbench and used in the BFM procedure calls. The default 
BFM configuration is defined with a clock period of C_UNDEFINED_TIME so that the BFM can detect and alert the user that the 
...
|                              |                              |                 | is defined by C_RGMII_VVC_MAX_INSTANCE_NUM      |
|                              |                              |                 | in adaptations_pkg.                             |
+------------------------------+------------------------------+-----------------+-------------------------------------------------+
| GC_RGMII_BFM_CONFIG          | :ref:`t_rgmii_bfm_config     | C_RGMII_BFM_CON\| Configuration for the RGMII BFM                 |
|                              | <t_rgmii_bfm_config>`        | FIG_DEFAULT     |                                                 |
+------------------------------+------------------------------+-----------------+-------------------------------------------------+
...
| Object   | Name               | Dir.   | Type                             | Description                                         |
+==========+====================+========+==================================+=====================================================+
| signal   | rgmii_vvc_tx_if    | inout  | :ref:`t_rgmii_tx_if <t_rgmii_if>`| RGMII TX signal interface record                    |
+----------+--------------------+--------+----------------------------------+-----------------------------------------------------+
| signal   | rgmii_vvc_rx_if    | inout  | :ref:`t_rgmii_rx_if <t_rgmii_if>`| RGMII RX signal interface record                    |
+----------+--------------------+--------+----------------------------------+-----------------------------------------------------+
...
| Record element               | Type                         | Default         | Description                                     |
+==============================+==============================+=================+=================================================+
| inter_bfm_delay              | :ref:`t_inter_bfm_delay`     | C_RGMII_INTER_B\| Delay between any requested BFM accesses        |
|                              |                              | FM_DELAY_DEFAULT| towards the DUT.                                |
|                              |                              |                 |                                                 |
...
|                              |                              | D_SEVERITY      |                                                 |
+------------------------------+------------------------------+-----------------+-------------------------------------------------+
| bfm_config                   | :ref:`t_rgmii_bfm_config     | C_RGMII_BFM_CON\| Configuration for the RGMII BFM                 |
|                              | <t_rgmii_bfm_config>`        | FIG_DEFAULT     |                                                 |
+------------------------------+------------------------------+-----------------+-------------------------------------------------+
...
.. note::
    ...
    be defined for the VVC framework. For this VVC, the following maximum values can be configured from adaptations_pkg:
      ...
      +--------------------------------------------+--------------------------------------+
      | C_RGMII_VVC_CMD_DATA_MAX_BYTES             | Maximum **data_array** length        |
      +--------------------------------------------+--------------------------------------+
      | C_RGMII_VVC_CMD_STRING_MAX_LENGTH          | Maximum **msg** length               |
      +--------------------------------------------+--------------------------------------+
...
rgmii_write()
----------------------------------------------------------------------------------------------------------------------------------
Adds a write command to the RGMII VVC executor queue, which will run as soon as all preceding commands have completed. When the 
command is scheduled to run, the executor calls the BFM :ref:`rgmii_write_bfm` procedure. data_array(0) is written first, while 
data_array(data_array'high) is written last.
...
rgmii_read()
----------------------------------------------------------------------------------------------------------------------------------
Adds a read command to the RGMII VVC executor queue, which will run as soon as all preceding commands have completed. When the 
command is scheduled to run, the executor calls the BFM :ref:`rgmii_read_bfm` procedure.
...
If the data_routing is set to TO_SB, the read data will be sent to the RGMII VVC dedicated scoreboard where it will be 
...
rgmii_expect()
----------------------------------------------------------------------------------------------------------------------------------
Adds an expect command to the RGMII VVC executor queue, which will run as soon as all preceding commands have completed. When the 
command is scheduled to run, the executor calls the BFM :ref:`rgmii_expect_bfm` procedure. The rgmii_expect() procedure will perform a 
...
.. table:: RGMII transaction info record fields. Transaction Type: t_base_transaction (BT) - accessible via **shared_rgmii_vvc_transaction_info.bt**
    ...
    +------------------------------+------------------------------+-----------------+-------------------------------------------------+
    | data_array                   | :ref:`t_byte_array(0 to 2047)| 0x0             | Data for RGMII read or write transaction        |
    |                              | <t_byte_array>`              |                 |                                                 |
    +------------------------------+------------------------------+-----------------+-------------------------------------------------+
...
i.e. rgmii_read(). Note that the data is only stored in the scoreboard and not accessible with the fetch_result() method when the 
TO_SB parameter is applied. The RGMII scoreboard is accessible from the testbench as a shared variable ``RGMII_VVC_SB``, located 
...
Scoreboard commands are available for the RGMII VVC scoreboard using the ``RGMII_VVC_SB``.
...
The unwanted activity detection is ignored when the rx_ctl signal goes low within one clock period after the VVC becomes inactive. 
...
.. table:: Compile order for the RGMII VVC
    ...
    +------------------------------+------------------------------------------------+-------------------------------------------------+
    | bitvis_vip_rgmii             | transaction_pkg.vhd                            | RGMII transaction package with DTT types,       |
    |                              |                                                | constants, etc.                                 |
    +------------------------------+------------------------------------------------+-------------------------------------------------+
    | bitvis_vip_rgmii             | vvc_cmd_pkg.vhd                                | RGMII VVC command types and operations          |
    +------------------------------+------------------------------------------------+-------------------------------------------------+
    | bitvis_vip_rgmii             | ../uvvm_vvc_framework/src_target_dependent/td\ | UVVM VVC target support package, compiled into  |
    |                              | _target_support_pkg.vhd                        | this VVC library                                |
    +------------------------------+------------------------------------------------+-------------------------------------------------+
    | bitvis_vip_rgmii             | ../uvvm_vvc_framework/src_target_dependent/td\ | Common UVVM framework methods compiled into the |
    ...
    +------------------------------+------------------------------------------------+-------------------------------------------------+
    | bitvis_vip_rgmii             | ../uvvm_vvc_framework/src_target_dependent/td\ | UVVM queue package for this VVC                 |
    |                              | _queue_pkg.vhd                                 |                                                 |
    +------------------------------+------------------------------------------------+-------------------------------------------------+
    | bitvis_vip_rgmii             | ../uvvm_vvc_framework/src_target_dependent/td\ | UVVM VVC entity support compiled into this      |
