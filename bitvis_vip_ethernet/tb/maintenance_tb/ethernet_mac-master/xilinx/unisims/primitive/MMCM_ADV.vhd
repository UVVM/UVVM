...
--   07/30/12 - 671160 669743 clock alignment and stopped behaviour
--   04/25/13 - 715228 - fix clkout_en0 time
--   04/26/13 - 708790, 709093 - vcoflag, period after DRP fixes
-- End Revision
...
----
  ...
  case clkfb_div_fint is
    ...
    clkout_en_val <= mf_product2 when (clkfb_frac_en = 1) else m_product;
...
--    rst_clkinstopped_tm <= '1' when ((clkout_en_t >= clkout_en_val - 3) and (clkout_en_t < clkout_en_val)) else '0';
    rst_clkinstopped_tm <= '1' when ((clkout_en_t + 3 >= clkout_en_val) and (clkout_en_t < clkout_en_val)) else '0'; -- 671160
      ...
      if (rising_edge(clkpll)) then
        if (clkfb_frac_en = 1 or clkvco_frac_en = 1) then
          if (pll_locked_tm = '1') then
            ...
            clkout_en_t <= clkout_en_t + 1; -- robh
            wait for period_vco_half;
            clkvco_lk_tmp <= '0';
          end if;
        else 
...
--                for I in 2 to md_product loop
                for L in 2 to m_product loop -- was 2 to
                  clkout_en_t <= L - 1; -- was -1
                  wait for (period_vco_half);
                  clkvco_lk_tmp <=  '0';
                  wait for (period_vco_half_rm);
                  clkvco_lk_tmp <=  '1';
...
-- dead CODE robh clk0_frac_out not used anywhere
...
--                  wait on rising_edge(clk0_sel_mux);
--                  clk0_frac_out <= '0';
--                  wait for clk0_frac_lt - 2 ps);
--                  wait on clk0_sel_mux;
--                  clk0_frac_out <= '1';
--                end loop;
--                wait for (clk0_frac_ht - 2 ps);
--                wait on clk0_sel_mux;
       ...
       if (clk0_frac_en = 0) then
        if (rising_edge(clk0in) or falling_edge(clk0in)) then
         ...
         if (clk0_frac_en = 1 and clkout_en = '1') then
                ...
                for I0 in 1 to 7 loop
                  wait for (clk0_frac_ht - 2 ps);
                  wait until rising_edge(clk0_sel_mux(0)) or rising_edge(clk0_sel_mux(1)) or rising_edge(clk0_sel_mux(2)) or rising_edge(clk0_sel_mux(3)) or rising_edge(clk0_sel_mux(4)) or rising_edge(clk0_sel_mux(5)) or rising_edge(clk0_sel_mux(6)) or rising_edge(clk0_sel_mux(7));
                  clk0_out <= '0';
                  wait for (clk0_frac_lt - 2 ps);
                  wait until rising_edge(clk0_sel_mux(0)) or rising_edge(clk0_sel_mux(1)) or rising_edge(clk0_sel_mux(2)) or rising_edge(clk0_sel_mux(3)) or rising_edge(clk0_sel_mux(4)) or rising_edge(clk0_sel_mux(5)) or rising_edge(clk0_sel_mux(6)) or rising_edge(clk0_sel_mux(7));
                  clk0_out <= '1';
      ...
      if (clkfb_frac_en = 0) then
        ...
        if (clkfb_frac_en = 1 and clkout_en = '1') then
                ...
                for Ifb in 1 to 7 loop
                  if (clkout_en = '1') then
                    wait for (clkfb_frac_ht - 2 ps);
                    wait until rising_edge(clkfb_sel_mux(0)) or rising_edge(clkfb_sel_mux(1)) or rising_edge(clkfb_sel_mux(2)) or rising_edge(clkfb_sel_mux(3)) or rising_edge(clkfb_sel_mux(4)) or rising_edge(clkfb_sel_mux(5)) or rising_edge(clkfb_sel_mux(6)) or rising_edge(clkfb_sel_mux(7));
                    clkfbm1_out <= '0';
                    wait for (clkfb_frac_lt - 2 ps);
                    wait until rising_edge(clkfb_sel_mux(0)) or rising_edge(clkfb_sel_mux(1)) or rising_edge(clkfb_sel_mux(2)) or rising_edge(clkfb_sel_mux(3)) or rising_edge(clkfb_sel_mux(4)) or rising_edge(clkfb_sel_mux(5)) or rising_edge(clkfb_sel_mux(6)) or rising_edge(clkfb_sel_mux(7));
                    clkfbm1_out <= '1';
...
--  clk_osc_p : process(clk_osc, rst_in) -- 671160
  clk_osc_p : process
  begin
    if (rst_in = '1') then
      clk_osc <= '0';
