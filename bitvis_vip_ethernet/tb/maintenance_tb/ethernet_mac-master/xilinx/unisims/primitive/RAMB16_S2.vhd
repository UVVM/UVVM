-- $Header: /devl/xcs/repo/env/Databases/CAEInterfaces/vhdsclibs/data/unisims/unisim/VITAL/RAMB16_S2.vhd,v 1.1 2008/06/19 16:59:26 vandanad Exp $
-------------------------------------------------------------------------------
...
-- /___/   /\     Filename : RAMB16_S2.vhd
-- \   \  /  \    Timestamp : Thu Apr  8 10:56:51 PDT 2004
--  \___\/\___\
--
-- Revision:
...
architecture RAMB16_S2_V of RAMB16_S2 is
...
begin
  VITALBehavior : process
    ...
    variable INIT_reg         : std_logic_vector (1 downto 0) := "00";
    variable SRVAL_reg         : std_logic_vector (1 downto 0) := "00";            
  ...
  begin
    if (first_time) then
      ...
      if (INIT'length > 2) then
        INIT_reg(1 downto 0) := To_StdLogicVector(INIT)(1 downto 0);
      else
        INIT_reg(INIT'length-1 downto 0) := To_StdLogicVector(INIT);
      end if;      
      DO <= INIT_reg(1 downto 0);
      
      if (SRVAL'length > 2) then
        SRVAL_reg(1 downto 0) := To_StdLogicVector(SRVAL)(1 downto 0);
      else
    ...
    if (rising_edge(CLK)) then
      if (EN = '1') then
        if (SSR = '1') then
          DO <= SRVAL_reg(1 downto 0) after 100 ps;
        else    
          if (WE = '1') then
            if (wr_mode = 0) then
              DO <= DI after 100 ps;
