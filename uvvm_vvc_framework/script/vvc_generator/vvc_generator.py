...
def add_vvc_interpreter(file_handle, vvc_channel):
    ...
    file_handle.write("    vvc_idx_for_activity_watchdog <= shared_inactivity_watchdog.priv_register_vvc(name      => " + vvc_name.upper() + ",\n")
...
def add_vvc_executor(file_handle, vvc_channel):
	...
	file_handle.write("      activity_watchdog_register_vvc_state(global_trigger_testcase_inactivity_watchdog, false, vvc_idx_for_activity_watchdog, last_cmd_idx_executed);\n")
	print_linefeed(file_handle)
	...
	print_linefeed(file_handle)
	file_handle.write("      -- Notify activity watchdog\n")
	file_handle.write("      activity_watchdog_register_vvc_state(global_trigger_testcase_inactivity_watchdog, true, vvc_idx_for_activity_watchdog, last_cmd_idx_executed);\n")
	print_linefeed(file_handle)
...
def add_methods_pkg_header(file_handle, vvc_name, vvc_channels):
    ...
    file_handle.write("  -- -- Activity Watchdog\n")
    file_handle.write("  -- --==============================================================================\n")
    file_handle.write("  -- procedure activity_watchdog_register_vvc_state( signal   global_trigger_testcase_inactivity_watchdog : inout std_logic;\n")
    ...
    file_handle.write("  --                                                 constant scope                                       : in string := \"vvc_register\");\n")    
    print_linefeed(file_handle)
...
def add_methods_pkg_body(file_handle, vvc_name):
    ...
    file_handle.write("  --                                                 constant vvc_idx_for_activity_watchdog               : in integer;\n")
    file_handle.write("  --                                                 constant last_cmd_idx_executed                       : in natural;\n")
    file_handle.write("  --                                                 constant scope                                       : in string := \"vvc_register\") is\n")
    file_handle.write("  -- begin\n")
    file_handle.write("  --   shared_inactivity_watchdog.priv_report_vvc_activity(vvc_idx               => vvc_idx_for_activity_watchdog,\n")
    file_handle.write("  --                                                       busy                  => busy,\n")
    file_handle.write("  --                                                       last_cmd_idx_executed => last_cmd_idx_executed);\n")
    file_handle.write("  --   gen_pulse(global_trigger_testcase_inactivity_watchdog, 0 ns, \"pulsing global trigger for inactivity watchdog\", scope, ID_NEVER);\n")
    file_handle.write("  -- end procedure;\n")
    print_linefeed(file_handle)
