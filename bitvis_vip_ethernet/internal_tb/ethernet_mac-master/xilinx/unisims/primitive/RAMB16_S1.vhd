-- $Header: /devl/xcs/repo/env/Databases/CAEInterfaces/vhdsclibs/data/unisims/unisim/VITAL/RAMB16_S1.vhd,v 1.1 2008/06/19 16:59:25 vandanad Exp $
-------------------------------------------------------------------------------
...
begin
  ...
  VITALBehavior : process
    ...
    variable INIT_reg         : std_logic_vector (0 downto 0) := "0";
    variable SRVAL_reg         : std_logic_vector (0 downto 0) := "0";            
  begin
    if (first_time) then
      ...
      if (INIT'length > 1) then
        INIT_reg(0 downto 0) := To_StdLogicVector(INIT)(0 downto 0);
      else
        INIT_reg(INIT'length-1 downto 0) := To_StdLogicVector(INIT);
      end if;      
      DO <= INIT_reg(0 downto 0);
      if (SRVAL'length > 1) then
        SRVAL_reg(0 downto 0) := To_StdLogicVector(SRVAL)(0 downto 0);
      else
    ...
    if (rising_edge(CLK)) then
      if (EN = '1') then
        if (SSR = '1') then
          DO <= SRVAL_reg(0 downto 0) after 100 ps;
        else    
          if (WE = '1') then
            if (wr_mode = 0) then
              DO <= DI after 100 ps;
