...
python dependencies/src/dependencies.py clone ssh://git@stash.bitvis.no/bv_vip/bitvis_vip_uart.git
# Copy Dockerfile to current workdir (required for making
# docker build include current workdir in the domain)
cp bitvis_vip_uart/internal_script/docker-simulation/Dockerfile ./
...
# Copy xunit report
docker cp bitvis_uart_unencrypted:/uvvm/bitvis_vip_uart/sim/xunit-report.xml ./bitvis_vip_uart/sim/xunit-report.xml
